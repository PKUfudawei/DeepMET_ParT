import torch, wandb, argparse, os, random, datetime, yaml
import torch.optim as optim
import numpy as np
import torch.distributed as dist

# official repository
from torch.nn.parallel import DistributedDataParallel as DDP
from torch.utils.data import DataLoader, DistributedSampler
from glob import glob
from tqdm import tqdm
from ema_pytorch import EMA

# self-used repository
from utils.data import Dataset, cycle
from utils.helper import fix_seeds, save_model, load_model
from models.particle_transformer import ParticleTransformer


def parse_arguments():
    parser = argparse.ArgumentParser(description='Hyper parameters and setup in training.')
    parser.add_argument('--train_data', type=str, default='data/train/DYJetsToMuMu_H2ErratumFix_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos/*.hdf5')
    parser.add_argument('--test_data', type=str, default='data/test/DYJetsToMuMu_H2ErratumFix_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos/*.hdf5')
    parser.add_argument('-g', '--gpus', type=str, default=None, help="comma-separated list of GPU ids to use, e.g. '0,2,3'")
    parser.add_argument('-b', '--batch_size', type=int, default=32)
    parser.add_argument('--max_PF_num', type=int, default=2**10)
    parser.add_argument('--lr', type=float, default=1e-4)
    parser.add_argument('--project', type=str, default='DeepMET_ParT')
    parser.add_argument('--total_epochs', type=int, default=10)
    parser.add_argument('--ema_start', type=int, default=100)
    parser.add_argument('--ema_decay', type=float, default=0.999)
    parser.add_argument('--use_wandb', type=bool, default=True)
    parser.add_argument('--num_workers', type=int, default=8)
    parser.add_argument('--pretrained_model', default=None)

    args = parser.parse_args()
    args.use_wandb = eval(args.use_wandb) if type(args.use_wandb) is str else args.use_wandb
    args.name = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    if args.gpus is not None:
        os.environ["CUDA_VISIBLE_DEVICES"] = args.gpus
        os.environ['OMP_NUM_THREADS'] = str(args.num_workers)
    
    os.makedirs(f'./checkpoints/{args.project}/{args.name}', exist_ok=True)
    with open(f'./checkpoints/{args.project}/{args.name}/setup.yaml', 'w') as f:
        yaml.dump(vars(args), f, default_flow_style=False)

    return args


def _check_finite(t, name):
    if not torch.isfinite(t).all():
        raise RuntimeError(f"[Non-finite detected] {name} contains NaN/Inf")


def train_batch(inputs, model, targets, optimizer, status='train', ema=None):
    _check_finite(inputs, "inputs")
    _check_finite(targets, "targets")
    if status == 'train':
        model.train()
        outputs = model(inputs)
        _check_finite(outputs, "outputs(train)")
        loss = torch.nn.functional.mse_loss(outputs, targets)
        _check_finite(loss, "loss(train)")
        optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 5.)
        optimizer.step()
        if ema is not None:
            ema.update()
    elif status == 'test':
        if ema is not None:
            ema_model = ema.ema_model
            ema_model.eval()
        else:
            model.eval()
        with torch.no_grad():
            outputs = ema_model(inputs) if ema is not None else model(inputs)
            _check_finite(outputs, "outputs(test)")
            loss = torch.nn.functional.mse_loss(outputs, targets)
            _check_finite(loss, "loss(test)")
    else:
        raise ValueError(f"Invalid status: {status}. Must be 'train' or 'test'.")
        
    return {f'{status}_loss': loss.item()}


if __name__ == "__main__":
    # --- Initialize DDP ---
    args = parse_arguments()
    use_single_card = (torch.cuda.device_count() <= 1)

    if use_single_card:
        gpu_rank = 0
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    else:
        gpu_rank = int(os.environ.get('LOCAL_RANK', 0))
        torch.cuda.set_device(gpu_rank)
        dist.init_process_group(backend="nccl")
        device = torch.device(f'cuda:{gpu_rank}')

    # Fix random seeds
    fix_seeds(seed=42)
    torch.set_default_dtype(torch.float32)
    torch.backends.cudnn.benchmark = True

    # Datasets
    train_files = glob(args.train_data)
    test_files = glob(args.test_data)
    dataset_train = Dataset(files=train_files, max_PF_num=args.max_PF_num, print_info=(gpu_rank==0))
    dataset_test = Dataset(files=test_files, max_PF_num=args.max_PF_num, print_info=(gpu_rank==0))

    train_sampler = DistributedSampler(dataset_train, shuffle=True) if not use_single_card else None
    test_sampler = DistributedSampler(dataset_test, shuffle=False) if not use_single_card else None

    dataloader_train = DataLoader(
        dataset=dataset_train, batch_size=args.batch_size, sampler=train_sampler, 
        num_workers=args.num_workers, pin_memory=True, drop_last=True, shuffle=(train_sampler is None),
    )
    dataloader_test = DataLoader(
        dataset=dataset_test, batch_size=10*args.batch_size, sampler=test_sampler,
        num_workers=args.num_workers, pin_memory=True, drop_last=False, shuffle=False,
    )
    test_loader = cycle(dataloader_test)

    # Model
    model = ParticleTransformer(
        input_dim=len(dataset_train.features), output_dim=len(dataset_train.truths),
        pair_input_dim=4, use_pre_activation_pair=False, embed_dims=[16, 64, 16],
        pair_embed_dims=[8, 8, 8], num_heads=4, num_layers=4, num_cls_layers=2, block_params=None,
        cls_block_params={'dropout': 0, 'attn_dropout': 0, 'activation_dropout': 0},
        fc_params=[], activation='gelu', trim=True, for_inference=False,
    ).to(device)
    if gpu_rank == 0:
        print(f'Model parameters: {sum(p.numel() for p in model.parameters()):,}')

    if args.pretrained_model is not None:
        model = load_model(model, args.pretrained_model)

    if not use_single_card:
        model = DDP(model, device_ids=[gpu_rank], output_device=gpu_rank, find_unused_parameters=True)

    ema = EMA(model if use_single_card else model.module, beta=args.ema_decay, update_after_step=args.ema_start, update_every=1)
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.0)

    # Only initialize wandb on the first GPU
    if args.use_wandb and gpu_rank == 0:
        wandb.init(project=args.project, mode='offline', name=args.name)

    # Training loop
    MIN_LOSS = 1e6
    for e in range(args.total_epochs):
        if not use_single_card:
            train_sampler.set_epoch(e)
            test_sampler.set_epoch(e)
        train_loader = tqdm(dataloader_train, desc=f"Epoch {e}") if gpu_rank == 0 else dataloader_train

        for index, (X, Y) in enumerate(train_loader):
            X, Y = X.to(device), Y.to(device)
            log_train = train_batch(X, model, Y, optimizer, status='train', ema=ema)

            if gpu_rank == 0:
                step = e * len(dataloader_train) + index
                if args.use_wandb:
                    wandb.log(log_train, step=step)

                # test
                if step % (len(dataloader_train)//20) == 0:
                    X_test, Y_test = next(test_loader)
                    X_test, Y_test = X_test.to(device), Y_test.to(device)
                    log_test = train_batch(X_test, model, Y_test, optimizer, status='test', ema=ema)
                    if log_test['test_loss'] < MIN_LOSS:
                        MIN_LOSS = log_test['test_loss']
                        save_model(ema.ema_model, epoch='best', output_save_dir=f'./checkpoints/{args.project}/{args.name}')
                    if args.use_wandb:
                        wandb.log(log_test, step=step)

        save_model(ema.ema_model, epoch=e+1, output_save_dir=f'./checkpoints/{args.project}/{args.name}')

    dataset_train.close()
    dataset_test.close()

    if args.use_wandb and gpu_rank == 0:
        wandb.finish()

    if not use_single_card:
        dist.destroy_process_group()
